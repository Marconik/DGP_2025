...
DijkstraSolverNoCheck(size_t startidx, size_t endidx, G& graph)
{
    ...
    std::cout << "Calc...\n";
    auto parsed =
        DijResParser(BasicDijkstraSolverNoCheck(startidx, graph), endidx);
    return { parsed };
}
    ...
    float& distance)
{
    ...
    for (MyMesh::EdgeIter ed = omesh.edges_begin(); ed != omesh.edges_end();
         ed++) {
    ...
    auto dijres = Dij::DijkstraSolverNoCheck(
        start_vertex_handle.idx(), end_vertex_handle.idx(), graph);
    ...
    if (dijres.has_value()) {
        ...
        for (auto v : res.seq)
            shortest_path_vertex_indices.push_back(v);
        distance = res.dist;
        return true;
    }
    return false;
